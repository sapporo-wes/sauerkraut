...
if [ ! -f ${DATA_DIR}/$sample.final.cram ] ; then
    echo "ERROR: ${DATA_DIR}/$sample.final.cram is required, but not found."
    exit
fi
...
if [ ! -f ${DATA_DIR}/${sample}_full.cram ] ; then
    ln -s ${DATA_DIR}/$sample.final.cram ${DATA_DIR}/${sample}_full.cram 
fi
...
if [ ! -f ${DATA_DIR}/${sample}_full.unmap.bam.bai ] ; then
    echo ""
    echo "----------------------------------------------"
    echo "Create index for full-ubam"
    cd ${DATA_DIR}
    cwltool --singularity \
	${SRC_DIR}/Tools/samtools-index-for-bam.cwl \
...
if [ ! -f ${DATA_DIR}/${sample}_mtsmall.cram ] ; then
    echo ""
    echo "----------------------------------------------"
    echo "Create mtsmall-cram"
    cd ${DATA_DIR}
    cwltool --singularity \
	${SRC_DIR}/Tools/samtools-extract-by-region-from-cram-to-cram.cwl \
	--cram ${DATA_DIR}/${sample}_full.cram \
	--reference ${REF_DIR}/Homo_sapiens_assembly38.fasta \
	--region chrM \
	--prefix ${sample}_mtsmall
    echo "...done."
    echo "----------------------------------------------"
    echo ""
fi

if [ ! -f ${DATA_DIR}/${sample}_mtsmall.cram.crai ] ; then
    echo ""
    echo "----------------------------------------------"
    echo "Create index for mtsmall-cram"
    cd ${DATA_DIR}
    cwltool --singularity \
	${SRC_DIR}/Tools/samtools-index-for-cram.cwl \
	--cram ${DATA_DIR}/${sample}_mtsmall.cram
    echo "...done."
    echo "----------------------------------------------"
    echo ""
fi

if [ ! -f ${DATA_DIR}/${sample}_mtsmall.bam ] ; then
    echo ""
    echo "----------------------------------------------"
    echo "Create mtsmall-bam"
    cd ${DATA_DIR}
    cwltool --singularity \
	${SRC_DIR}/Tools/samtools-convert-cram-to-bam.cwl \
	--cram ${DATA_DIR}/${sample}_mtsmall.cram \
...
if [ ! -f ${DATA_DIR}/${sample}_mtsmall.bam.bai ] ; then
    echo ""
    echo "----------------------------------------------"
    echo "Create index for mtsmall-bam"
    cd ${DATA_DIR}
    cwltool --singularity \
	${SRC_DIR}/Tools/samtools-index-for-bam.cwl \
	--bam ${DATA_DIR}/${sample}_mtsmall.bam
    echo "...done."
    echo "----------------------------------------------"
    echo ""
fi
